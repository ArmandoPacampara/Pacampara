...
// --- 1. Get Movie ID First (Moved to Top) ---
...
// --- 2. Fetch AI Suggestions (Passing the movie_id) ---
function getBestViewingTimes($movie_id) {
    // Append movie_id query parameter
...
// Now we fetch suggestions specifically for THIS movie
$ai_suggestions = getBestViewingTimes($movie_id);

// --- 3. Fetch Movie Details (Standard PHP Logic) ---
  ...
  <body class="ml-20 mr-20 mt-10 mb-10">
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 gap-4">
                        <?php foreach ($ai_suggestions as $index => $slot): 
                            ...
                            $badge = match($index) {
                                0 => '<span class="bg-yellow-400 text-yellow-900 text-xs font-bold px-2 py-1 rounded">BEST CHOICE</span>',
                                default => ''
                            };
    ...
    <div class="w-full h-[500px] mb-10">
      ...
      <div class="h-[400px] p-5">
        <img class="rounded-2xl object-cover w-full h-full" src="../../../../public/assets/images/<?= $movie_poster ?>" alt="<?= $movie_name ?> Poster" />
      </div>
    </div>
